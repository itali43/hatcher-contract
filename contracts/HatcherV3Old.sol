  ...
  OwnableUpgradeable
{
  ...
  function withdrawAndSplitHalfsies(
    ...
    uint remaining = address(this).balance - (halfBalance * 2);

    // Transfer half the balance to each address
    payable(address1).transfer(halfBalance);
    payable(address2).transfer(halfBalance);

    // If there is 1 wei remaining, send it to one of the addresses
    if (remaining > 0) {
      payable(address1).transfer(remaining);
    }
  }
